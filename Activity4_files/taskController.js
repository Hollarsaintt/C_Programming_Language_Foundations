...
app.service('Languages', ['$rootScope', function ($rootScope) {
   ...
   this.allSourceLanguages = [
      ...
      {id: 'shell', label: "Shell", ext: 'sh', ace: {mode: 'sh'}, defaultDisabled: true},
      {id: 'text', label: "Text", ext: 'txt', ace: {mode: 'text'}, evalAs: 'shell', defaultDisabled: true},
      {id: 'michelson', label: "Michelson", ext: 'tz', ace: {mode: 'text'}, defaultDisabled: true},
      {id: 'ligo', label: "PascaLIGO", ext: 'ligo', ace: {mode: 'text'}, defaultDisabled: true},
      {id: 'mligo', label: "CameLIGO", ext: 'mligo', ace: {mode: 'text'}, defaultDisabled: true},
      {id: 'religo', label: "ReasonLIGO", ext: 'religo', ace: {mode: 'text'}, defaultDisabled: true},
      {id: 'jsligo', label: "JsLIGO", ext: 'jsligo', ace: {mode: 'text'}, defaultDisabled: true},
      {id: 'smartpy', label: "SmartPy", ext: 'py', ace: {mode: 'python'}, defaultDisabled: true},
      {id: 'archetype', label: "Archetype", ext: 'arl', ace: {mode: 'text'}, defaultDisabled: true},
...
app.controller('taskController', ['$scope', '$http', 'FioiEditor2Tabsets', 'FioiEditor2Signals', 'FioiEditor2Recorder', 'PEMApi', '$sce', '$rootScope', 'TabsetConfig', '$timeout', '$interval', '$window', 'Languages', '$uibModal', function($scope, $http, tabsets, signals, recorder, PEMApi, $sce, $rootScope, TabsetConfig, $timeout, $interval, $window, Languages, $uibModal) {
   ...
   } else if(getParameterByName('theme') == 'coursera') {
      window.disableFullscreen = true;
      $scope.bodyClass = 'coursera';
   }
   ...
   if (!$scope.standaloneMode) {
      ...
      if($rootScope.sPlatform == 'http://algorea.pem.dev' && !inIframe()) {
         $rootScope.sPlatform = 'miniPlatform';
      }
   ...
   $scope.initSourcesEditorsData = function() {
      ...
      if (!hasSourceCode) {
         ...
         var newText = defaultSources && defaultSources[Languages.defaultLanguage] ? defaultSources[Languages.defaultLanguage] : '';
      ...
      $scope.saveSubmission(null, true, function() {
            ...
            platform.validate('done', function() {
               ...
               if(msg == 'timeout_error') {
                  $scope.validateErr = "L'évaluation a pris trop de temps. Cela signifie en général que votre programme met trop de temps à résoudre le problème.";
               } else if(msg) {
   ...
   $scope.loadSubmissionInEditor = function(submission) {
      ...
      var code = sourcesTabset.addTab().update({title: 'Code1'});
      code.getBuffer().update({text: source_code.sSource, language: source_code.params.sLangProg});
   };

   // Set a stateReloaded at the start
...
/*      $scope.logValidate('task:getAnswer');
   ...
   function updateStringsFromSync(strings) {
      ...
      $timeout(function() {
         angular.forEach(document.getElementsByTagName('code'), function(c) {
            if(!$(c).hasClass('noPrism')) {
